...
def _get_task_status(trellis_dir: Path, hook_input: dict) -> str:
    ...
    if active.stale or not task_dir.is_dir():
        return (
            ...
            "Next: Task directory not found. Run: C:\Users\limin\AppData\Roaming\uv\python\cpython-3.12-windows-x86_64-none\python.exe ./.trellis/scripts/task.py finish"
        )
    ...
    if task_status == "completed":
        return (
            f"Status: COMPLETED\nTask: {task_title}\n"
            f"Next: Archive with `C:\Users\limin\AppData\Roaming\uv\python\cpython-3.12-windows-x86_64-none\python.exe ./.trellis/scripts/task.py archive {task_dir.name}` "
            "or start a new task."
        )
...
def _build_compact_current_state(
    ...
    if get_tasks_dir and iter_active_tasks:
        try:
            ...
            lines.append(
                f"Active tasks: {task_count} total. Use `C:\Users\limin\AppData\Roaming\uv\python\cpython-3.12-windows-x86_64-none\python.exe ./.trellis/scripts/task.py list --mine` only if needed."
            )
        except Exception:
...
def _build_workflow_toc(workflow_path: Path) -> str:
    ...
    out_lines = [
        ...
        "Full guide: .trellis/workflow.md. Step detail: `C:\Users\limin\AppData\Roaming\uv\python\cpython-3.12-windows-x86_64-none\python.exe ./.trellis/scripts/get_context.py --mode phase --step <X.Y>`.",
        "",
    ]
